...
def scale_sigmoid_activation_function(tensor: torch.Tensor, alpha: int or float):
    """
    :param tensor: a torch tensor, range is [-1, 1]
    :param alpha: an scale parameter to calculate exp(-aplha*tensor)/(1+exp(-alpha*tensor))
    :return: mapping tensor to [0, 1]
    """
    assert torch.all(tensor.ge(-1)) and torch.all(tensor.le(1)), "Out of range!"
    alpha = torch.tensor(alpha, dtype=torch.float32, device=tensor.device)
    one = torch.tensor(1, dtype=torch.float32, device=tensor.device)
    neg_one = torch.tensor(-1, dtype=torch.float32, device=tensor.device)
    tensor = torch.sigmoid(torch.mul(alpha, tensor))
    max_value = torch.sigmoid(torch.mul(alpha, one))
    min_value = torch.sigmoid(torch.mul(alpha, neg_one))
    output = torch.div(torch.sub(tensor, min_value), torch.sub(max_value, min_value))
    return output
